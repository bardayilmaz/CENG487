...
# objects
cyl = Cylinder(0.5, 1.0, [Mat3d.get_rotation_x_matrix(60), Mat3d.get_rotation_y_matrix(360/5)], Vec3d(0.0, -0.5, -2.50, 1), 18, 1)

cube = Cube([Mat3d.get_rotation_z_matrix(15), Mat3d.get_rotation_x_matrix(60), Mat3d.get_rotation_y_matrix(90)], Vec3d(0.0, 0.0, -5.0, 1), 2, 1)

pyramid = Pyramid([Mat3d.get_rotation_z_matrix(23), Mat3d.get_rotation_x_matrix(40), Mat3d.get_rotation_y_matrix(15)], Vec3d(0.0, 0.0, -5.0, 1), 1, 1)

sphere = Sphere(0.5, 36, 18, [Mat3d.get_rotation_x_matrix(90), Mat3d.get_rotation_y_matrix(1)], Vec3d(0.0, 0.0, -3.0, 1), 1)

torus = Torus(0.5, 0.5, 16, 8, [Mat3d.get_rotation_x_matrix(90), Mat3d.get_rotation_y_matrix(1)], Vec3d(0.0, 0.0, -3.0, 1), 1)

shapes = np.array([pyramid, cube, cyl, torus, sphere])
current_index = 0
...
#Camera
camera = Camera(Vec3d(0, 0, 0, 1), Vec3d(0, 0, -1, 1))
...
def DrawGLScene():
    ...
    gluLookAt(0.0, 0.0, 4.0, 0.0, 0.0, -4.0, 0.0, 1.0, 0.0)
    scene.draw_objects()
...
def keyPressed(key, x, y):
    ...
    if key == b'p':
        ...
        scene = Scene(shapes[current_index].objects)
    if key == b'+':
        shapes[current_index].subdivision_level += 1
        shapes[current_index].re_init()
        scene = Scene(shapes[current_index].objects)
